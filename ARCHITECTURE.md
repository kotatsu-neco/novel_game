...
## v02 Text Display Architecture

```text
SCENARIO_SOURCE / main.json
  ↓
inline command normalization [r] / [p]
  ↓
japaneseLayoutRules.js
  ↓
textMeasure.js
  ↓
textPaginator.js
...
JLREQ参照の実用禁則処理。行頭禁止、行末禁止、ページ頭禁止、孤立禁止の判定を持つ。

### textMeasure.js

非表示のDOMを使い、実際の表示領域に収まるかを測定する。

### textPaginator.js

測定結果と禁則処理に基づき、本文 / 台詞 / 文書をページ配列に変換する。

### typewriterController.js

requestAnimationFrameで文字送りを制御する。
