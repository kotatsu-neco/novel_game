...
- `tools/publish_game.py` を追加し、ゲーム部分のみの軽量publish zipを作成可能にした。
- メニューに表示設定UIを追加し、本文・台詞・文書の文字送りスピードを独立設定可能にした。
- 本文・台詞・文書のフォントサイズを独立設定できる基礎を追加した。
- choice stepに `timeLimitMs` / `timeoutNext` / `timeoutChoiceLabel` を追加し、回答時間制限を実装した。
- 縦書きは未対応。将来別工程。


## No-sample engine package

This package intentionally does not include sample scenario data.  
Install a scenario pack before opening `index.html`.

See `SCENARIO_INSTALL.md`.
