...
- publish_game.py追加。
- 表示設定UI追加。
- text / voice / document の文字送り速度を独立設定可能にした。
- text / voice / document のフォントサイズ設定基礎を追加した。
- 選択肢時間制限機能を追加した。
- validator / route graph checker を timed choice に対応させた。
- 縦書きは未実装。
...
- 日本語禁則処理モジュールを追加。
- DOM測定ベースのページ分割モジュールを追加。
- requestAnimationFrameベースの文字送り制御へ移行。
- `[r]` / `[p]` 処理を仕様化。
- ResizeObserverによる再ページ分割基礎を追加。
- 文字サイズ変更時の再ページ分割基礎を追加。
- 禁則検査スクリプトとテストケースを追加。
